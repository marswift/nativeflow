  ...
  useEffect(() => {
    ...
    async function checkSession() {
      try {
        ...
        if (!sessionError && session?.user && isActive) {
    ...
    try {
      ...
      if (signInError) {
        ...
        const isInvalidCredentials =
          msg.includes('invalid') && (msg.includes('credentials') || msg.includes('login') || msg.includes('password'))
        const isEmailNotConfirmed = msg.includes('confirm') || msg.includes('verified') || msg.includes('email not')
        ...
        <main className="flex-1">
          <div className={CONTAINER_CLASS}>
            <div className="text-center mt-6 sm:mt-8">
              <h1 className="text-2xl font-bold tracking-tight text-[#1a1a2e] sm:text-3xl">ログイン</h1>
              <p className="mt-2 text-sm text-[#4a4a6a]">メールアドレスとパスワードでログインしてください。</p>
            </div>
            ...
            <p className="mt-6 text-center text-sm text-[#4a4a6a]">
              初めての方は
              <Link href="/signup" className="ml-1 font-semibold text-amber-600 hover:text-amber-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:ring-offset-2 rounded">新規登録</Link>
              へ
            </p>
          </div>
        </main>
