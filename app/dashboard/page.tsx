...
type TrialUrgency = 'normal' | 'warning' | 'urgent'
...
function computeTrialUrgency(diffMs: number): TrialUrgency {
  const dayMs = 24 * 60 * 60 * 1000
  if (diffMs < dayMs) return 'urgent'
  if (diffMs < 3 * dayMs) return 'warning'
  return 'normal'
}

function getTrialDisplay(profile: DashboardProfileRow | null, currentTimeMs?: number | null): TrialDisplay {
  const empty: TrialDisplay = { showTrialCard: false, isExpired: false, isPaymentMissingAfterTrial: false, headline: '', subline: '', urgency: 'normal', showCta: false }
  ...
  // App-level trial (trial_ends_at, independent of Stripe)
  if (trialEndsAt && subscriptionStatus !== 'active' && subscriptionStatus !== 'past_due') {
    const diffMs = typeof now === 'number' ? getDiffMs(trialEndsAt, now) : getDiffMs(trialEndsAt)
    if (diffMs > 0) {
      const urgency = computeTrialUrgency(diffMs)
      return {
        showTrialCard: true, isExpired: false, isPaymentMissingAfterTrial: false,
        headline: `無料トライアル中 ${formatRemainingLabel(diffMs)}`,
        subline: 'トライアル期間中はすべての機能をお試しいただけます。\n期間終了後も継続するにはプランをご契約ください。',
        urgency,
        showCta: true,
      }
    }
  ...
  if (subscriptionStatus === 'canceled' || subscriptionStatus === 'unpaid') {
    return { showTrialCard: true, isExpired: true, isPaymentMissingAfterTrial: true, headline: 'お支払い状況をご確認ください', subline: '契約状況または継続利用の設定をお支払い・契約管理ページから確認できます。', urgency: 'urgent', showCta: true }
  }
  return empty
}

function DashboardLoadingState({ loadingMessage, onLogout }: {
  loadingMessage: string; onLogout: () => void
}) {
  return (
...
function DashboardErrorState({ message, onLogout }: {
  message: string; onLogout: () => void
}) {
  return (
    <div className={PAGE_SHELL_CLASS} style={{ fontFamily: "'Nunito','Hiragino Sans',sans-serif" }}>
      <AppHeader onLogout={onLogout} />
...
export default function DashboardPage() {
    ...
    async function loadDashboard() {
      ...
      try {
        ...
        const { data: row, error: fetchError } = await supabase
          .from('user_profiles')
          .select('id, daily_study_minutes_goal, current_streak_days, best_streak_days, last_streak_date, avatar_character_code, avatar_level, avatar_image_url, planned_plan_code, subscription_status, subscription_current_period_end, trial_ends_at, total_flow_points, total_diamonds, username, current_learning_language, target_language_code')
          .eq('id', userId)
          .maybeSingle()
      ...
      // Run independent post-profile queries in parallel
      const [statResult, historyResult, announcementResult] = await Promise.all([
        // 1. Today's daily stat + lesson_runs fallback
        (async () => {
          ...
          try {
            ...
            stat = (statRow as DailyStatRow) ?? null
        ...
        (async () => {
          try {
            ...
            const { data: urgentRow } = await supabase
              ...
              .limit(1)
              .maybeSingle()
            return (urgentRow as { id: string; title: string; published_at: string } | null) ?? null
          } catch { return null }
        })(),
      ])

      if (isActive) {
        setTodayStat(statResult)
        setStreakDays(computeStreak(getTodayStatDate(), historyResult))
        setUrgentAnnouncement(announcementResult)
        setLoading(false)
      }
    }
  ...
  if (loading) return <DashboardLoadingState loadingMessage="読み込み中です..." onLogout={handleLogout} />
  if (!profile || pageError) return <DashboardErrorState message="データの読み込みに失敗しました" onLogout={handleLogout} />
      ...
      <main className="flex-1">
        <div className="mx-auto w-full max-w-6xl px-6 pt-8 pb-10 sm:px-8 sm:pt-10 lg:px-10">
          ...
          {trialDisplay.showTrialCard && (
            <section className="mx-auto mt-6 max-w-lg" aria-label="無料トライアル">
              <div className={
                trialDisplay.isExpired && trialDisplay.isPaymentMissingAfterTrial
                  ? `w-full ${RADIUS} bg-gradient-to-br from-white via-red-50/70 to-amber-50/80 px-5 py-4 ${CARD_SHADOW} border border-red-200/60 shadow-sm`
                  : trialDisplay.urgency === 'urgent'
                    ? `w-full ${RADIUS} bg-gradient-to-br from-red-50 via-amber-50 to-red-50/80 px-5 py-4 ${CARD_SHADOW} border border-red-200/60`
                    : trialDisplay.urgency === 'warning'
                      ? `w-full ${RADIUS} bg-gradient-to-br from-amber-100 via-amber-50 to-orange-50/80 px-5 py-4 ${CARD_SHADOW} border border-amber-300/60`
                      : `w-full ${RADIUS} bg-gradient-to-br from-amber-100 via-amber-50 to-amber-100/80 px-5 py-4 ${CARD_SHADOW} border border-amber-200/60`
              }>
                <div className="flex flex-col items-center text-center">
                  {/* Urgency indicator dot */}
                  <div className="mb-2 flex items-center gap-2">
                    <span className={`inline-block h-2 w-2 rounded-full ${
                      trialDisplay.urgency === 'urgent' || trialDisplay.isExpired ? 'bg-red-500 animate-pulse' :
                      trialDisplay.urgency === 'warning' ? 'bg-amber-500' : 'bg-amber-400'
                    }`} />
                    <span className={`text-[11px] font-bold uppercase tracking-wider ${
                      trialDisplay.urgency === 'urgent' || trialDisplay.isExpired ? 'text-red-600' :
                      trialDisplay.urgency === 'warning' ? 'text-amber-700' : 'text-amber-600'
                    }`}>
                      {trialDisplay.isExpired ? 'ACTION REQUIRED' : 'FREE TRIAL'}
                    </span>
                  </div>

                  <p className={
                    trialDisplay.isExpired
                  ...
                  {trialDisplay.subline && (
                    <p className={`mt-2 text-xs leading-relaxed whitespace-pre-line text-center ${
                      trialDisplay.isExpired ? 'text-red-700/80' :
                      trialDisplay.urgency === 'urgent' ? 'text-red-700/80' : 'text-amber-700/90'
                    }`}>
                      {trialDisplay.subline}
                    </p>
                  )}
                  ...
                  {trialDisplay.dangerNote && (
                    <p className="mt-2 text-[11px] font-medium text-red-600 leading-snug">{trialDisplay.dangerNote}</p>
                  )}

                  {trialDisplay.showCta && (
                    <Link
                      href="/settings/billing"
                      className={`mt-3 inline-flex items-center justify-center rounded-xl px-5 py-2.5 text-sm font-bold shadow-sm transition hover:-translate-y-px focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                        trialDisplay.urgency === 'urgent' || trialDisplay.isExpired
                          ? 'bg-red-600 text-white hover:bg-red-700 focus:ring-red-400'
                          : 'bg-amber-500 text-white hover:bg-amber-600 focus:ring-amber-400'
                      }`}
                    >
                      {trialDisplay.isExpired ? 'お支払い・契約を確認する' : 'プランを確認する'}
                    </Link>
                  )}
                </div>
              </div>
          ...
          <section className="mt-8 flex justify-center">
            <div className="grid w-full max-w-5xl grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <DashboardCard title="Rewards" description="ダイヤを使って特典を獲得" href="/rewards" icon="💎" />
          ...
          <section className="mt-8">
            <div className={`${RADIUS} ${CARD_BORDER} bg-gradient-to-br from-white via-[#FFFDF8] to-[#FFF9EC] ${CARD_SHADOW} px-5 py-5 sm:px-6`}>
              <div className="mb-4 flex items-center justify-between">
                <p className="text-[13px] font-bold tracking-[0.06em] text-[#7b7b94]">REWARDS</p>
                <p className="text-[10px] text-[#9ca3af]">学習するたびにポイントが貯まります</p>
              </div>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {/* Diamonds */}
                <div className="flex flex-col items-center rounded-2xl border border-[#E8E4DF] bg-white px-4 py-4">
                  <div className="flex items-center gap-1.5">
                    <span className="text-lg" aria-hidden="true">💎</span>
                    <span className="text-2xl font-extrabold tracking-tight text-[#1a1a2e]">{profile.total_diamonds ?? 0}</span>
                  </div>
                  <p className="mt-1 text-xs font-bold text-[#5a5a7a]">ダイヤモンド</p>
                  <p className="mt-0.5 text-[10px] leading-snug text-[#9ca3af]">報酬・特典に使える</p>
                </div>
                {/* FlowPoints */}
                <div className="flex flex-col items-center rounded-2xl border border-[#E8E4DF] bg-white px-4 py-4">
                  <div className="flex items-center gap-1.5">
                    <span className="text-lg" aria-hidden="true">🔥</span>
                    <span className="text-2xl font-extrabold tracking-tight text-[#1a1a2e]">{displayedTotalFlowPoints}</span>
                  </div>
                  <p className="mt-1 text-xs font-bold text-[#5a5a7a]">Flowポイント</p>
                  <p className="mt-0.5 text-[10px] leading-snug text-[#9ca3af]">あと{pointsToNextRank}ptで次のランク</p>
                </div>
                {/* Streak */}
                <div className="flex flex-col items-center rounded-2xl border border-[#E8E4DF] bg-white px-4 py-4">
                  <div className="flex items-center gap-1.5">
                    <span className="text-lg" aria-hidden="true">⚡</span>
                    <span className="text-2xl font-extrabold tracking-tight text-[#1a1a2e]">{displayStreak}</span>
                  </div>
                  <p className="mt-1 text-xs font-bold text-[#5a5a7a]">継続日数</p>
                  <p className="mt-0.5 text-[10px] leading-snug text-[#9ca3af]">毎日の積み重ねが力になる</p>
                </div>
                {/* Today's study */}
                <div className="flex flex-col items-center rounded-2xl border border-[#E8E4DF] bg-white px-4 py-4">
                  <div className="flex items-center gap-1">
                    <span className="text-lg" aria-hidden="true">📖</span>
                    <span className="text-2xl font-extrabold tracking-tight text-[#1a1a2e]">{studyMinutesActual}<span className="ml-0.5 text-sm font-medium text-[#8a8a9a]">分</span></span>
                  </div>
                  <p className="mt-1 text-xs font-bold text-[#5a5a7a]">今日のレッスン</p>
                  <p className="mt-0.5 text-[10px] leading-snug text-[#9ca3af]">目標 {studyGoal}分 / {todayLessonRunsCompleted}回完了</p>
                  {dailyStatsError && <p className="mt-1 text-[10px] text-amber-600">{DAILY_STATS_ERROR}</p>}
                </div>
              </div>
            </div>
          </section>
